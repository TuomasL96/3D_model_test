...
func cycle_rotation_axis():
	var axis_int = int(rotation_axis)
	axis_int = (axis_int + 1) % 3
	rotation_axis = RotationAxis.values()[axis_int]

func rotate_device(event: InputEventMouseMotion, device: Node3D, rotation_axis: RotationAxis):
	...
	match rotation_axis:
		...
		RotationAxis.y:
			...
			device.rotate(device.transform.basis.x, motion.y * -device_y_sensitivity/1000)
		RotationAxis.z:
			device.rotate_z(motion.x * -device_x_sensitivity/1000)
			device.rotate(device.transform.basis.x, motion.y * -device_y_sensitivity/1000)
